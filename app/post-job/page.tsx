  ...
  useEffect(()=>{
    supabase.auth.getUser().then(async({data})=>{
      ...
      if(editId){
        ...
        if(job&&job.employer_id===data.user.id){
          setCover(job.cover_image_url||'')
          setLogo(job.company_logo_url||'')
          setForm({
            title:job.title||'',
            location:job.location||'',
            type:job.type||'Vor Ort',
            contract:job.contract||'Vollzeit',
            level:job.level||'Mid',
            field:job.field||'Sonstiges',
  ...
  return(
    <AppShell>
      <div className="topbar">
        ...
        <span className="topbar-title" style={{flex:1}}>{editId?'Stelle bearbeiten':'Stelle inserieren'}</span>
      </div>
      <div style={{maxWidth:660,margin:'0 auto',padding:'1.5rem'}}>
        {err&&<div className="alert alert-err">{err}</div>}
        {ok&&<div className="alert alert-ok">✓ {editId?'Stelle aktualisiert!':'Stelle live!'} Weiterleitung...</div>}
        <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          <button type="submit" className="f-sub" disabled={loading||ok||uploading}>
            {uploading?'⏳ Bild lädt...':loading?'⏳ Wird aktualisiert...':editId?'💾 Änderungen speichern':'🚀 Stelle live schalten'}
          </button>
        </form>
      </div>
    </AppShell>
