...
export default function KIToolsPage() {
  ...
  const findNearbyJobs = async (score: number, jobDescription: string) => {
    if (score < 50) return // Nur bei gutem Match
    try {
      // Keywords aus Jobdescription extrahieren (erste paar Wörter)
      const keywords = jobDescription.split(' ').slice(0, 3).join(' ')
      let query = supabase.from('jobs').select('*').eq('is_active', true).limit(4)

      // Stadtfilter wenn angegeben
      if (city.trim()) {
  ...
  const matchJob = async () => {
    ...
    try {
      ...
      // Passende Jobs in der Umgebung suchen
